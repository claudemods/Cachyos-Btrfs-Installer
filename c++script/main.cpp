...
void configure_installation() {
    ...
    if (ROOT_PASSWORD.empty()) {
        ROOT_PASSWORD = run_command("dialog --title \"Root Password\" --passwordbox \"Enter root password (min 4 chars):\" 10 50 2>&1 >/dev/tty");
    }
    ...
    if (LOCALE_LANG == "en_GB.UTF-8") {
        LOCALE_LANG = run_command("dialog --title \"Locale\" --inputbox \"Enter locale (e.g. UTF-8):\" 10 50 2>&1 >/dev/tty");
    }
